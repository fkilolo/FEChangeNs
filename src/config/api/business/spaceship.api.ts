...
export const callFetchSpaceshipDomains = (connectId: string, params: { take?: number; skip?: number; orderBy?: string }) => {
  const { take = 50, skip = 0, orderBy = "name" } = params;
  return axios.get(`/api/v1/spaceship/domains/${connectId}?take=${take}&skip=${skip}&orderBy=${orderBy}`);
};
