...
import { Select, Spin, Input, Button, message, Alert, Flex, Transfer, Table, Pagination } from "antd";
...
const TableTransfer: React.FC<TableTransferProps & {
  ...
  const { leftColumns, rightColumns, targetKeys, dataSource, leftPage, setLeftPage, leftPageSize, setLeftPageSize, leftTotal, rightPage, setRightPage, rightPageSize, setRightPageSize, ...restProps } = props;
  // Data bảng phải (selected)
  const rightData = dataSource.filter(item => targetKeys.includes(item.key));
  const rightTotal = rightData.length;
  const rightDataPaged = rightData.slice((rightPage - 1) * rightPageSize, rightPage * rightPageSize);
...
const NameServer = () => {
  ...
  useEffect(() => {
    ...
    const fetchDomains = async () => {
      ...
      try {
        ...
        const res = await callFetchSpaceshipDomains(selectedId, { take: leftPageSize, skip, orderBy: "name" });
        let items = (res?.items || []).map((item: any) => ({ key: String(item.name), name: item.name }));
    ...
    fetchDomains();
  }, [selectedId, leftPage, leftPageSize]);
  ...
  return (
    <div>
      <h1>Name Server</h1>
      <Flex gap={8} align="center" style={{ marginBottom: 16, justifyContent: 'space-between' }}>
